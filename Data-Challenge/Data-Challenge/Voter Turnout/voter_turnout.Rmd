...
```{r}
#Creation of variables for use in a bar graph
ml_vt_before <- ml_vt_all %>%
  filter(election_year <= 2016)
ml_vt_before_mean <- mean(ml_vt_before$voter_turnout)
ml_vt_after <- ml_vt_all %>%
  filter(election_year > 2016)
ml_vt_after_mean <- mean(ml_vt_after$voter_turnout)

#setting up x values for bar graph
vt_bar_x <- c('RI Before Parolee Enfranchisement', 'RI After Parolee Enfranchisement', 'MD Before Parolee Enfranchisement', 'MD After Parolee Encfranchisement')

#setting up y values for bar graph
vt_bar_y <- c(ri_vt_before_mean, ri_vt_after_mean, ml_vt_before_mean, ml_vt_after_mean)

vt_bar_df <- data.frame(vt_bar_x, vt_bar_y)

#Creation of bar graph containing comparision of before and after parolee enfranchisement in two different states
ggplot(vt_bar_df, aes(x = vt_bar_x, y = vt_bar_y)) +
  geom_bar(stat = "Identity") +
  theme_light() +
  labs(x = '', y = 'Average Number of Votes', title = 'Comparison of Average Voter Turnout Before and After Parolee Enfranchisement', subtitle = 'Data obtained from Rhode Island Department of State and Maryland State Board of Elections')
```
